...
def parse_arguments():
    parser = argparse.ArgumentParser(description='''
        USDM Comformance Rules Test Data Template Generator. Reads the Enterprise Architect XMI export file
        containing definition of the USDM UML model (specified in the -x option) and the USDM Controlled
        Terminology Excel file (specified in the -c option) and creates an Excel template file (optionally
        specified in the -o option) for test data entry.''')
    parser.add_argument("-x", "--xmi_file", help="USDM XMI export file (e.g., <DDF version>/Deliverables/UML/USDM_UML.xmi)",required=True)
    parser.add_argument("-c", "--ct_file", help="USDM CT Excel file (e.g., <DDF version>/Deliverables/CT/USDM_CT.xlsx)",required=True)
    parser.add_argument("-o", "--output_file", help="[Optional] Specifies output file Excel file. Default is ./USDM_Test_Data_Template.xlsx",default="USDM_Test_Data_Template.xlsx")
...
def get_properties(entName:str,cls: Tag):
    ...
    for prp in cls.find_all("ownedAttribute",attrs={"xmi:type":"uml:Property"}):
        ...
        typs += [attr.properties['type']]
        crds += [get_card(attr.bounds)]
    for prpn in (k for k,v  in entdict[entName]["Properties"].items() if v["Role"] == "Attribute" and not cls.find("ownedAttribute",attrs={"xmi:type":"uml:Property","name":k})):
        print(f"Attribute '{entName}.{prpn}' defined in {args.ct_file} does not have a matching class attribute in {args.xmi_file}")
    if entName != "Study":
    ...
    # Add a blank row with defined format to prevent auto-copying of format from row above.
...
for ctrow in ctws.iter_rows(min_row=2):
    ...
    if elrole == "Entity":
        ...
        entdict[entname]["Properties"][elname] = {"Role":elrole,"NCI C-code":ctrow[4].value,"Preferred Name": ctrow[5].value,"Definition":ctrow[7].value,"CodelistRef": cref.group(1) if cref else None}

workbook = xlsxwriter.Workbook(args.output_file)
usdmver = usdmxmi.find("properties",{"name":"USDM","type":"Logical"}).find_parent("diagram").project['version']
